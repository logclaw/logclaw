...
<Tabs>
  <Tab title="LogClaw Cloud (Managed)">
    ...
    Set these environment variables — **all OTel SDKs auto-detect them**, no endpoint changes in code:
  ...
  <Tab title="Self-Hosted">
    ...
    Set the environment variable — **all OTel SDKs auto-detect it**, no endpoint changes in code:
...
loggerProvider.addLogRecordProcessor(
  new BatchLogRecordProcessor(new OTLPLogExporter())  // reads OTEL_EXPORTER_OTLP_ENDPOINT
);
...
func main() {
    ...
    exporter, _ := otlploggrpc.New(ctx)  // reads OTEL_EXPORTER_OTLP_ENDPOINT
...
# Zero code changes — the agent reads OTEL_EXPORTER_OTLP_ENDPOINT automatically:
...
builder.Logging.AddOpenTelemetry(logging =>
{
    ...
    logging.AddOtlpExporter();  // reads OTEL_EXPORTER_OTLP_ENDPOINT
});
...
# Zero code changes — reads OTEL_EXPORTER_OTLP_ENDPOINT automatically:
rails server
...
fn init_logs() -> LoggerProvider {
    let exporter = LogExporter::builder()
        .with_tonic()   // reads OTEL_EXPORTER_OTLP_ENDPOINT
        .build()
...
<Note>
The examples below **auto-detect** your endpoint from the `OTEL_EXPORTER_OTLP_ENDPOINT` environment variable set above. Switch between **LogClaw Cloud** and **Self-Hosted** by changing that single env var — no code changes needed.
</Note>
...
java -javaagent:opentelemetry-javaagent.jar \
  ...
  -Dotel.logs.exporter=otlp \
  -jar my-app.jar
# The agent reads OTEL_EXPORTER_OTLP_ENDPOINT and OTEL_EXPORTER_OTLP_HEADERS automatically
```
...
# Endpoint and headers are read from OTEL_EXPORTER_OTLP_ENDPOINT / OTEL_EXPORTER_OTLP_HEADERS env vars
...
```properties
# application.properties
# Endpoint read from OTEL_EXPORTER_OTLP_ENDPOINT env var
  ...
  c.add_span_processor(
    OpenTelemetry::SDK::Trace::Export::BatchSpanProcessor.new(
      OpenTelemetry::Exporter::OTLP::Exporter.new(
        endpoint: ENV['OTEL_EXPORTER_OTLP_ENDPOINT'] || 'https://otel.logclaw.ai'
      )
    )
  )
end
...
```bash
# Reads OTEL_EXPORTER_OTLP_ENDPOINT set above — just run your app:
rails server
```
...
$exporter = new LogsExporter(
    (new \OpenTelemetry\Contrib\Otlp\OtlpHttpTransportFactory())->create(
        getenv('OTEL_EXPORTER_OTLP_ENDPOINT') . '/v1/logs',
        'application/json'
    )
);
...
<Note>
Log shippers use **config files**, not OTel SDKs, so they cannot auto-detect `OTEL_EXPORTER_OTLP_ENDPOINT`. Set the endpoint explicitly in each config. The examples below use the **LogClaw Cloud** URL — for self-hosted, replace with `http://localhost:4318`.
</Note>
...
exporters:
  otlphttp:
    endpoint: "${OTEL_EXPORTER_OTLP_ENDPOINT}"   # set once, works for Cloud or Self-Hosted
    headers:
      x-logclaw-api-key: "${LOGCLAW_API_KEY}"     # only needed for LogClaw Cloud
