...
<Note>
The examples below use the **self-hosted** endpoint (`otel.logclaw.ai`). For **LogClaw Cloud**, set these environment variables instead of changing endpoints in code:
...
3. Verify OTLP endpoint URL — common mistakes:
   ...
   - **LogClaw Cloud:** using `otel.logclaw.ai` instead of `console.logclaw.ai/api/ingest`
