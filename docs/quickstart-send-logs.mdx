...
<Tabs>
  <Tab title="LogClaw Cloud (Managed)">
    | Protocol | URL |
    |----------|-----|
    | HTTP | `https://console.logclaw.ai/api/ingest/v1/logs` |
...
#   OTEL_EXPORTER_OTLP_ENDPOINT=https://console.logclaw.ai/api/ingest
...
java -javaagent:opentelemetry-javaagent.jar \
  -Dotel.exporter.otlp.endpoint=https://console.logclaw.ai/api/ingest \
...
```bash
export OTEL_EXPORTER_OTLP_ENDPOINT=https://console.logclaw.ai/api/ingest
...
1. Test connectivity:
   ...
   <Tabs>
     <Tab title="LogClaw Cloud">
       ```bash
       curl -v https://console.logclaw.ai/api/ingest/v1/logs \
         -H "Content-Type: application/json" \
         -H "x-logclaw-api-key: $LOGCLAW_API_KEY" \
         -d '{"resourceLogs":[]}'
...
3. Verify OTLP endpoint URL — common mistakes:
   ...
   - **LogClaw Cloud:** using `localhost` instead of `console.logclaw.ai/api/ingest`
