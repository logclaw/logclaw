...
# For self-hosted, use: endpoint="localhost:4317", insecure=True
# LogClaw Cloud requires an API key via env var:
...
provider.add_log_record_processor(
    BatchLogRecordProcessor(
        OTLPLogExporter(endpoint="otel.logclaw.ai:443")
    )
)
...
// For self-hosted, use: url: 'http://localhost:4318'
// LogClaw Cloud requires an API key via env var:
//   OTEL_EXPORTER_OTLP_HEADERS=x-logclaw-api-key=lc_proj_your_key_here
...
loggerProvider.addLogRecordProcessor(
  new BatchLogRecordProcessor(
    new OTLPLogExporter({ url: 'https://otel.logclaw.ai' })
  )
);
...
// For self-hosted, use: WithEndpoint("localhost:4317"), WithInsecure()
// LogClaw Cloud requires an API key via env var:
//   OTEL_EXPORTER_OTLP_HEADERS=x-logclaw-api-key=lc_proj_your_key_here
package main
...
// For self-hosted, use: new Uri("http://localhost:4318")
// LogClaw Cloud requires an API key via env var:
//   OTEL_EXPORTER_OTLP_HEADERS=x-logclaw-api-key=lc_proj_your_key_here
...
builder.Logging.AddOpenTelemetry(logging =>
{
    ...
    {
        options.Endpoint = new Uri("https://otel.logclaw.ai");
    });
});
...
# LogClaw Cloud (managed) — zero code changes with auto-instrumentation:
...
// For self-hosted, use: .with_endpoint("http://localhost:4318")
// LogClaw Cloud requires an API key via env var:
//   OTEL_EXPORTER_OTLP_HEADERS=x-logclaw-api-key=lc_proj_your_key_here
...
fn init_logs() -> LoggerProvider {
    let exporter = LogExporter::builder()
        .with_tonic()
        .with_endpoint("https://otel.logclaw.ai")
        .build()
...
<Note>
The examples below use the **LogClaw Cloud** endpoint (`otel.logclaw.ai`). For **self-hosted** deployments, replace the endpoint with `localhost:4317` (gRPC) or `http://localhost:4318` (HTTP) and add `insecure=True` where applicable.

**LogClaw Cloud** requires an API key — set it via environment variable:

```bash
export OTEL_EXPORTER_OTLP_HEADERS=x-logclaw-api-key=lc_proj_your_key_here
```

See [API Keys](/api-keys) to get your key.
</Note>
...
exporters:
  otlphttp:
    endpoint: https://otel.logclaw.ai
    headers:
      x-logclaw-api-key: "${LOGCLAW_API_KEY}"
