...
<Tabs>
  <Tab title="LogClaw Cloud (Managed)">
    ...
    | gRPC | `grpcs://otel.logclaw.ai:443` |

    Every code example below includes this endpoint and your API key **inline** — just copy and run. See [API Keys](/api-keys) to create your key.
  ...
  <Tab title="Self-Hosted">
    ...
    Every code example below defaults to the **LogClaw Cloud** endpoint. For self-hosted, swap the URL to `http://localhost:4318` and remove the `x-logclaw-api-key` header — look for the `# Self-hosted` comments in each snippet.
...
```bash cURL
# LogClaw Cloud — replace lc_proj_... with your API key
# Self-hosted: swap URL to http://localhost:4318/v1/logs and remove the api-key header
...
provider.add_log_record_processor(
    BatchLogRecordProcessor(OTLPLogExporter(
        endpoint="https://otel.logclaw.ai",                      # Self-hosted: http://localhost:4317
...
loggerProvider.addLogRecordProcessor(
  new BatchLogRecordProcessor(new OTLPLogExporter({
    url: 'https://otel.logclaw.ai',                                    // Self-hosted: http://localhost:4317
    headers: { 'x-logclaw-api-key': 'lc_proj_your_key_here' },        // Self-hosted: remove this line
  }))
);
...
func main() {
    ...
    exporter, _ := otlploggrpc.New(ctx,
        otlploggrpc.WithEndpointURL("otel.logclaw.ai:443"),                                        // Self-hosted: localhost:4317
        otlploggrpc.WithHeaders(map[string]string{"x-logclaw-api-key": "lc_proj_your_key_here"}),  // Self-hosted: remove
    )
...
# Zero code changes — just pass the endpoint and API key as JVM flags:
java -javaagent:opentelemetry-javaagent.jar \
  ...
  -jar my-app.jar
# Self-hosted: change endpoint to http://localhost:4318 and remove the headers flag
...
builder.Logging.AddOpenTelemetry(logging =>
{
    ...
    {
        o.Endpoint = new Uri("https://otel.logclaw.ai");          // Self-hosted: http://localhost:4318
        o.Headers = "x-logclaw-api-key=lc_proj_your_key_here";   // Self-hosted: remove this line
    });
});
...
fn init_logs() -> LoggerProvider {
    let exporter = LogExporter::builder()
        .with_tonic()
        .with_endpoint("https://otel.logclaw.ai")   // Self-hosted: http://localhost:4317
        .with_metadata(metadata_with_api_key())      // Self-hosted: remove this line
        .build()
...
fn metadata_with_api_key() -> tonic::metadata::MetadataMap {
    let mut meta = tonic::metadata::MetadataMap::new();
    meta.insert("x-logclaw-api-key", "lc_proj_your_key_here".parse().unwrap());
    meta
}
```
...
<Note>
The examples below include the **LogClaw Cloud** endpoint and API key inline — just copy and run. For **self-hosted** deployments, swap `https://otel.logclaw.ai` → `http://localhost:4318` (HTTP) or `localhost:4317` (gRPC) and remove the `x-logclaw-api-key` header. Look for the `# Self-hosted` comments in each snippet.
</Note>
...
def configure_logging(**kwargs):
    ...
    provider.add_log_record_processor(BatchLogRecordProcessor(OTLPLogExporter(
        endpoint="https://otel.logclaw.ai",                      # Self-hosted: http://localhost:4317
        headers={"x-logclaw-api-key": "lc_proj_your_key_here"},  # Self-hosted: remove
    )))
    logging.getLogger().addHandler(LoggingHandler(logger_provider=provider))
...
# Run your Spring Boot app with the agent — endpoint and API key passed as JVM flags:
java -javaagent:opentelemetry-javaagent.jar \
  -Dotel.exporter.otlp.endpoint=https://otel.logclaw.ai \
  -Dotel.exporter.otlp.headers=x-logclaw-api-key=lc_proj_your_key_here \
  -Dotel.service.name=my-spring-api \
...
otel.exporter.otlp.endpoint=https://otel.logclaw.ai
otel.exporter.otlp.headers=x-logclaw-api-key=lc_proj_your_key_here
otel.service.name=my-spring-api
otel.logs.exporter=otlp
# Self-hosted: change endpoint to http://localhost:4318 and remove the headers line
```
...
```properties
# application.properties
quarkus.otel.exporter.otlp.endpoint=https://otel.logclaw.ai
quarkus.otel.exporter.otlp.headers=x-logclaw-api-key=lc_proj_your_key_here
...
otel:
  exporter:
    otlp:
      endpoint: https://otel.logclaw.ai                            # Self-hosted: http://localhost:4318
      headers:
        x-logclaw-api-key: lc_proj_your_key_here                   # Self-hosted: remove this line
  resource:
    attributes:
...
fun main() {
    // Run with: java -javaagent:opentelemetry-javaagent.jar
    //   -Dotel.exporter.otlp.endpoint=https://otel.logclaw.ai
    //   -Dotel.exporter.otlp.headers=x-logclaw-api-key=lc_proj_your_key_here -jar app.jar
...
$exporter = new LogsExporter(
    (new \OpenTelemetry\Contrib\Otlp\OtlpHttpTransportFactory())->create(
        'https://otel.logclaw.ai/v1/logs',      // Self-hosted: http://localhost:4318/v1/logs
        'application/json',
        ['x-logclaw-api-key' => 'lc_proj_your_key_here']  // Self-hosted: remove this line
    )
);
