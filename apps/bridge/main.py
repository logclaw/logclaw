...
# ---------------------------------------------------------------------------
# Logging — OTEL-instrumented, with JSON-stdout fallback
# ---------------------------------------------------------------------------
def _setup_otel_logging():
    try:
        ...
        resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "logclaw-bridge"),
        })
...
# Worker thread references — populated in main(); checked by /health
...
def main():
    ...
    _worker_threads.update({t.name: t for t in threads})
