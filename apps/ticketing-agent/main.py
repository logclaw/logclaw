...
dedup_registry = {}  # dedup_key -> {"ticket_id": str, "expires": float, "trace_count": int}

def find_groupable_ticket(dedup_key):
    """Find an existing ticket that this event can be grouped into (max 3 traces)."""
    now = time.time()
...
def append_trace_to_ticket(entry, event):
    """Append a new trace to an existing ticket (up to 3 traces)."""
...
def process(event):
    ...
    # Build dedup key — include error pattern hash for request_failure
    ...
    # Check for existing ticket to group into (up to 3 traces)
    ...
    create_ticket(event, dedup_key)


def create_ticket(event, dedup_key):
    """Create a new TICK-NNNN ticket from an anomaly event."""
    now = now_iso()
    svc = event.get("service", "unknown")
    atype = event.get("anomaly_type", "unknown")
    ...
    # Build title based on anomaly type
    if atype == "request_failure":
    ...
    # Build request traces list (for request_failure events)
    request_traces = []
    ...
    # Generate reproduce steps from request trace
    reproduce_steps = []
    if event.get("request_trace"):
    ...
    # For error_rate_spike, still gather evidence logs from OpenSearch
    ...
    # Register in dedup registry for grouping
