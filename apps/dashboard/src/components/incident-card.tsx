...
export default function IncidentCard({ incident, selected, onSelect }: IncidentCardProps) {
  return (
    <div className={`card-hover group rounded-2xl bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06),0_1px_2px_rgba(0,0,0,0.04)] ${selected ? "ring-2 ring-[#FF5722] ring-offset-1" : ""}`}>
      <div className="flex items-start p-4 sm:p-5">
        {onSelect && (
          <button
            ...
            className="mr-3 mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border-2 border-[#d1d1d6] transition-all hover:border-[#FF5722]"
          >
            {selected && (
              <div className="h-3 w-3 rounded-sm bg-[#FF5722]" />
            )}
          </button>
        )}
        <Link
          href={`/incidents/${incident.id}`}
          className="flex min-w-0 flex-1 items-start justify-between gap-3 sm:gap-4"
        >
